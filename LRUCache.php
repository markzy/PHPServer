...
class LRUCache {
    ...
    public function get($key) {
        ...
        if (count($this->hashmap) == 1) { return $node ;}
...
class Node {
    ...
    //the last change time of a file
    private $last_ctime;
     ...
     */
    public function __construct($key, $data, $date = 0) {
        $this->key = $key;
        $this->data = $data;
        $this->last_ctime = $date;
    }
    ...
    public function isoutofdate(){
        $newdate = filectime($this->getKey());
        if($newdate != $this->last_ctime) {
            $this->last_ctime = $newdate;
            return true;
        }
        else return false;
    }

//    public function set_ctime($time){
//        $this->last_ctime = $time;
//    }
